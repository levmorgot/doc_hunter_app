...
abstract class IDepartmentRemoteDataSource {
  Future<List<DepartmentModel>> getAllDepartments(int limit, int skip);

  Future<List<DepartmentModel>> searchDepartment(String query, int limit, int skip);
}
...
class DepartmentRemoteDataSource implements IDepartmentRemoteDataSource {
  ...
  @override
  Future<List<DepartmentModel>> getAllDepartments(int limit, int skip) async {
    return await _getDepartmentsFromUrl(
        'http://89.108.83.99:8000/departments/?limit=$limit&skip=$skip');
  }
  ...
  Future<List<DepartmentModel>> searchDepartment(
    ...
    return await _getDepartmentsFromUrl(
        'http://89.108.83.99:8000/departments/?search_string=$query&limit=1000&skip=$skip');
  }
